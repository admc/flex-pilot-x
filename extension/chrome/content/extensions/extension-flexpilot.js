...
Selenium.prototype.doFlexDragDropElemToElem = function(locator, flashLoc, option) {
    var movie = this.browserbot.findElement(locator);
    var flashObj = parse_locator(flashLoc);
    ...
    flashParam[flashObj.type] = flashObj['string'];
    flashParam[optObj.type] = optObj['string'];
    var res = movie['fp_dragDropElemToElem'](flashParam);
    try {
        var msg = res.message;
        throw msg;
    } catch(err){}
...
//x.doFlashDragDropElemToElem("id=devMovie", "chain=name:dragSprite", "optchain=id:subPanel")
Selenium.prototype.doFlexDragDropToCoords = function(locator, flashLoc, coords) {
    var movie = this.browserbot.findElement(locator);
    var flashObj = parse_locator(flashLoc);
    ...
    var res = movie['fp_dragDropToCoords'](flashParam);
    try {
        var msg = res.message;
        throw msg;
    } catch(err){}
